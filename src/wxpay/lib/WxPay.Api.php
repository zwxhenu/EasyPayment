...
class WxPayApi
{
     ...
     * 支付结果通用通知
     *
     * @param callable $callback 直接回调函数使用方法: notify(you_function);
    ...
    public static function notify($callback, &$msg)
    {
        ...
        // 如果返回成功则验证签名
        try {
            $result = WxPayResults::Init($xml, false);
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        if ($data) {
            ...
            curl_close($ch);
            throw new WxPayException("curl出错，错误码:$error 错误信息：$error_info" . print_r($xml, true));
        }
    }
